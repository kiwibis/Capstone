...
function editDistance(source, target) {
  ...
  for (let row = 0; row <= lengthTarget; row++) {
    distances.push(Array(lengthSource + 1).fill(0))
  }
  for (let row = 0; row <= lengthTarget; row++) {
    //row = i
    for (let column = 0; column <= lengthSource; column++) {
      ...
      if (row === 0) {
        if (column !== 0) {
          distances[row][column] = distances[row][column - 1] + 1
        }
        continue
      } else if (column === 0) {
        ...
        distances[row][column] = Math.min(
          distances[row - 1][column] + weightOfInsert,
          distances[row][column - 1] + weightOfDelete,
          distances[row - 1][column - 1] + operationCost
        )
      }
    }
  }
  return distances[lengthTarget][lengthSource]
}
